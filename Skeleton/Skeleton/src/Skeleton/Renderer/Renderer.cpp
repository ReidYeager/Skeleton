...
{
	vkWaitForFences(vulkanContext.device, 1, &backend->flightFences[backend->currentFrame], VK_TRUE, UINT64_MAX);

	uint32_t imageIndex;
	vkAcquireNextImageKHR(vulkanContext.device, backend->swapchain, UINT64_MAX, backend->imageAvailableSemaphores[backend->currentFrame], VK_NULL_HANDLE, &imageIndex);

	if (backend->imageIsInFlightFences[imageIndex] != VK_NULL_HANDLE)
	{
		vkWaitForFences(vulkanContext.device, 1, &backend->imageIsInFlightFences[imageIndex], VK_TRUE, UINT64_MAX);
	}
	...
	SKL_ASSERT_VK(
		vkQueueSubmit(vulkanContext.graphicsQueue, 1, &submitInfo, backend->flightFences[backend->currentFrame]),
...
void Renderer::RecordCommandBuffers()
{
	...
	for (uint32_t i = 0; i < comandCount; i++)
	{
		...
		for (uint32_t j = 0; j < vulkanContext.renderables.size(); j++)
		{
			...
			vkCmdBindDescriptorSets(backend->commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, parProg->pipelineLayout, 0, 1, &parProg->descriptorSet, 0, nullptr);
			...
			vkCmdDrawIndexed(backend->commandBuffers[i], static_cast<uint32_t>(mesh.indices.size()), 1, 0, 0, 0);
		}
	...
	VkImageLayout _new)
{
	...
	EndSingleTimeCommand(transitionCommand, backend->graphicsCommandPool, vulkanContext.graphicsQueue);
}
