...
inline const char* VulkanResultToString(VkResult _result)
{
	...
	ETS(VK_ERROR_INCOMPATIBLE_VERSION_KHR);
	...
	//ETS(VK_ERROR_OUT_OF_POOL_MEMORY_KHR);				// Re-definition
	//ETS(VK_ERROR_INVALID_EXTERNAL_HANDLE_KHR);		// Re-definition
	//ETS(VK_ERROR_FRAGMENTATION_EXT);					// Re-definition
	//ETS(VK_ERROR_INVALID_DEVICE_ADDRESS_EXT);			// Re-definition
	//ETS(VK_ERROR_INVALID_OPAQUE_CAPTURE_ADDRESS_KHR);	// Re-definition
	//ETS(VK_ERROR_PIPELINE_COMPILE_REQUIRED_EXT);		// Re-definition
...
{															\
	...
	{														\
		...
		char sklAssertBuffer[100];							\
		sprintf_s(sklAssertBuffer, "Throwing %s", VulkanResultToString(vkAssertResult));\
		throw sklAssertBuffer;								\
	}														\
}
} // namespace skeleton
